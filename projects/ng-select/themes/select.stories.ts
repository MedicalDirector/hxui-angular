...
@Component({
  ...
  selector: 'sb-storybook-multiple',
  template: `
    <form [formGroup]="form">
      <ng-select
        [items]="people$ | async"
...
class StorybookMultipleComponent {
...
class StorybookSingleComponent {
  people$: Observable<Person[]>;
  peopleInput$ = new Subject<string>();
...
export default {
  ...
  decorators: [
    moduleMetadata({
      ...
      declarations: [StorybookMultipleComponent, StorybookSingleComponent],
...
const TemplateMultiple: Story = () => {
  return {
    template: `
      <sb-storybook-multiple></sb-storybook-multiple>
    `,
  };
};

export const MultipleModern = TemplateMultiple.bind({});
MultipleModern.play = async ({ canvasElement }) => {
  // get input
...
const TemplateSingle: Story = () => {
  return {
    template: `
      <sb-storybook-single></sb-storybook-single>
    `,
  };
};

export const SingleModern = TemplateSingle.bind({});
SingleModern.play = async ({ canvasElement }) => {
  // get input
  const canvas = within(canvasElement);
  const input = canvas.getByRole('combobox');
